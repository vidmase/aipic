...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isFreeUser) {
      // Free users: only allow free models
      ...
      // --- Quota Check ---
      // Count images generated by this user in the last 24 hours
      const since = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
      const { count: imagesLast24h, error: countError } = await supabase
        .from("generated_images")
        .select("id", { count: "exact", head: true })
        .eq("user_id", session.user.id)
        .gte("created_at", since)
      if (countError) {
        console.error("Error counting user images for quota:", countError)
        return NextResponse.json({ error: "Failed to check quota" }, { status: 500 })
      }
      if ((imagesLast24h ?? 0) + num_images > IMAGE_GENERATION_QUOTA_PER_DAY) {
        return NextResponse.json({
          error: `Quota exceeded: You can generate up to ${IMAGE_GENERATION_QUOTA_PER_DAY} images per 24 hours. Please try again later.`
        }, { status: 429 })
      }
    }
